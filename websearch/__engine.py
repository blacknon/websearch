...
# TODO(blacknon): このファイルに集約しすぎてこのままだと見にくいので、検索エンジンや処理ごとに分離する (優先度:`高`)
#                 => 共有している処理は関数化して、ファイルごとに分けるか？
#                 => set系については、jsonを別途用意してそれを読み込ませるようにしたほうがいいかな？

# TODO(blacknon): スクレイピングではなく、APIを介しての検索をオプションで指定できるようにする

# TODO(blacknon): 画像検索もちゃんと追加する

# TODO(blacknon): BANされた時にわかるようにする(errを返すようにしたい) (優先度:`高`)
#                 => とりあえず、取得したサンプルを元にReCaptchaが出てるかどうかを識別させる
...
#                      - https://github.com/chr0x6eos/ReCaptchaBypass#                      - https://qiita.com/derodero24/items/7d36f4617a40fbb36b11

# TODO(blacknon): Seleniumへの対応が必要(オプションの追加) (優先度:`高`)
...
class SearchEngine:
    ...
    # 外部関数化のため、selfで持っている各変数をdict化して保持しておき、それを各検索エンジンのファイルに渡す方式にするか？
    def set(self, engine):
        ''' サーチエンジンごとの設定 '''
        # Baidu
        if engine == 'baidu':
    ...
    def get_links(self, html, type):
        ...
        if self.ENGINE == 'Google' and self.SPLASH_URL == "":
            new_elinks = []
            for elink in elinks:
        ...
        if self.ENGINE == 'Baidu':
            ...
            elinks = loop.run_until_complete(
                common.resolv_links(loop, self.session, elinks))
            loop.close()
